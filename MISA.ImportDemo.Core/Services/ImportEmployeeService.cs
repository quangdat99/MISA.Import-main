...
namespace MISA.ImportDemo.Core.Services
{
    ...
    public class ImportEmployeeService : BaseImportService, IImportEmployeeService
    {
        ...
        {
            if ((entity is Employee && importColumn.ColumnInsert == "DateOfBirth"))
            {
                if (cellValue.GetType() == typeof(double))
                {
                    var year = Convert.ToInt32((double)cellValue);
                    return new DateTime(year, 1, 1);
                } else
                {
                    return  base.GetProcessDateTimeValue(entity, cellValue, type, importColumn);
                }
            } else if (entity is EmployeeFamily && importColumn.ColumnInsert == "DateOfBirth")
            {
